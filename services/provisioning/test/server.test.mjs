...
test('HTTP enrollment creates payload then consumes token once', async () => {
  await withServer(async base => {
    const created = await fetch(`${base}/v1/enrollments`, {
      ...
      body: JSON.stringify({ policyProfile: 'corporate', provisioningMode: 'work-profile', ttlSeconds: 60 }),
    }).then(r => r.json());
    ...
    assert.equal(payload.payload['android.app.extra.PROVISIONING_ADMIN_EXTRAS_BUNDLE']['io.dpcaio.extra.PROVISIONING_MODE'], 'work-profile');
