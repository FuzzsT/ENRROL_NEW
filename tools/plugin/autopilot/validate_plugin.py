...
Vendored compatibility snapshot aligned with the installed Plugin Autopilot
contract and official OpenAI plugin packaging docs verified 2026-08-22.
"""
...
def _component_path(root: Path, manifest: dict, field: str, expected: str, errors: list[str]) -> bool:
...
def _svg_size(path: Path) -> tuple[float, float]:
    ...
    if view_box:
        values = [float(x) for x in re.split(r"[\s,]+", view_box.strip()) if x]
...
def _image_size(path: Path) -> tuple[float, float]:
    ...
    if suffix == ".png": return _png_size(data)
    if suffix in {".jpg", ".jpeg"}: return _jpeg_size(data)
    if suffix == ".webp": return _webp_size(data)
...
def _walk(root: Path, errors: list[str], exclusions: list[str]) -> tuple[list[Path], int, int]:
    ...
    for current, dirs, names in os.walk(root, topdown=True, followlinks=False):
        ...
        for name in names:
            ...
            try: mode = path.lstat().st_mode
                ...
                _error(errors, f"unreadable plugin member {rel}: {exc}"); continue
            if stat.S_ISLNK(mode):
                _error(errors, f"symlink is not allowed in public plugin: {rel}"); continue
            if not stat.S_ISREG(mode):
                _error(errors, f"unsupported plugin member type: {rel}"); continue
            size = path.stat().st_size
            files.append(path); total += size
            if size > MAX_MEMBER: _error(errors, f"plugin member exceeds 100 MiB: {rel}")
            base = path.name
    ...
    if entries > MAX_ENTRIES: _error(errors, f"plugin would exceed 5000 archive entries: {entries}")
    if total > MAX_TOTAL: _error(errors, f"plugin extracted size exceeds 512 MiB: {total}")
    return sorted(files), entries, total


def validate_plugin(plugin_root: str, exclusions: list[str] | None = None) -> dict:
    root = Path(plugin_root).expanduser().resolve()
    exclusions = sorted({x for x in (exclusions or []) if x})
    errors: list[str] = []
    warnings: list[str] = []
    if not root.is_dir():
    ...
    manifest = _load_json(manifest_path, errors) if manifest_path.is_file() else {}
    if not manifest_path.is_file(): _error(errors, "missing .codex-plugin/plugin.json")
    ...
    if isinstance(author, dict): _public_https(author.get("url"), "author.url", errors, 2048)
    ...
    skill_value = manifest.get("skills", "./skills/")
    skills: list[str] = []
    if not isinstance(skill_value, str):
        _error(errors, "manifest skills must be a relative path string")
    else:
        if not skill_value.startswith("./"): _error(errors, f"manifest skills path must start with ./: {skill_value}")
        rel = skill_value[2:] if skill_value.startswith("./") else skill_value
        if rel.rstrip("/") != "skills": _error(errors, f"manifest skills path must resolve to ./skills/: {skill_value}")
        skill_root = root / "skills"
        if skill_root.is_dir():
            for directory in sorted((x for x in skill_root.iterdir() if x.is_dir()), key=lambda x: x.name):
                definition = directory / "SKILL.md"
                if not definition.is_file():
                    _error(errors, f"skill directory is missing SKILL.md: {directory.name}"); continue
                try: skill_name, skill_desc = _skill_metadata(definition)
                except Exception as exc:
                    _error(errors, f"skill definition unreadable: {directory.name}: {exc}"); continue
                if skill_name != directory.name: _error(errors, f"skill name must match directory: {directory.name}")
                if not skill_desc: _error(errors, f"skill description is required: {directory.name}")
                elif len(skill_desc.strip("\"'")) > 1024: _error(errors, f"skill description exceeds 1024 characters: {directory.name}")
                text = definition.read_text(encoding="utf-8")
                end = text.find("\n---", 4)
                body = text[end+4:].strip() if end >= 0 else ""
                if not body: _error(errors, f"skill body must not be empty: {directory.name}")
                ...
                if directory.name.startswith("."): _error(errors, f"skill directory must not be hidden: {directory.name}")
                skills.append(directory.name)
        elif "skills" in manifest:
            _error(errors, f"manifest skills path is missing: {skill_value}")

    has_skills = bool(skills)
    architecture = "hybrid" if has_mcp and has_skills else "MCP-backed" if has_mcp else "skills-only"
    if not has_skills and not has_mcp: _error(errors, "plugin must contain at least one Skill or an MCP-backed capability")

    interface = manifest.get("interface")
    if not isinstance(interface, dict):
        _error(errors, "manifest interface is required and must be an object"); interface = {}
    for field, limit in (("displayName",30),("shortDescription",30),("longDescription",4000),("developerName",80)):
        value = interface.get(field)
        if not isinstance(value, str) or not value: _error(errors, f"interface.{field} is required")
        elif len(value) > limit: _error(errors, f"interface.{field} exceeds final directory limit of {limit} characters")
    ...
    if not isinstance(category, str) or not category: _error(errors, "interface.category is required for final directory submission")
    elif category not in CATEGORIES: _error(errors, f"interface.category is unsupported: {category}")
    ...
    if capabilities is not None:
        ...
        elif any(not isinstance(x,str) or not x or len(x)>120 or "\n" in x or "\r" in x for x in capabilities):
    ...
    if prompts is not None:
        ...
        if prompt_list is None: _error(errors, "interface.defaultPrompt must be a string or list of strings")
        else:
            if len(prompt_list) > 3: _error(errors, "interface.defaultPrompt must contain at most 3 prompts")
            seen: set[str] = set()
            for prompt in prompt_list:
                if not isinstance(prompt, str) or not prompt.strip():
                    _error(errors, "each interface.defaultPrompt must be a non-empty string"); continue
                ...
                if norm in seen: _error(errors, "interface.defaultPrompt entries must be unique after normalization")
                seen.add(norm)
    ...
    if not exclusions: warnings.append("no explicit public exclusions supplied; confirm the repository has no internal-only capabilities")
    return {
        "ok": not errors,
        "pluginRoot": str(root),
