...
if 'versionName = "1.3.0"' not in app: errors.append('versionName 1.3.0 missing')
m=re.search(r'versionCode\s*=\s*(\d+)', app)
if not m or int(m.group(1)) < 27: errors.append('versionCode >=27 required')
...
if 'DPC-AIO 1.3.0' not in readme or 'QR Release Bundle' not in readme:
    errors.append('README 1.3.0 QR release highlights missing')

report=read('RELEASE-VERIFICATION.json')
try:
    obj=json.loads(report)
    if obj.get('version')!='1.3.0': errors.append('release report version !=1.3.0')
...
if 'test_114_qr_release_bundle_contract.py' not in runner:
    errors.append('1.3.0 contract not wired into host tests')
